...
export class CierresController {
  ...
  @Post('cerrar')
  cerrarCaja(@Body() data: any) {
    return this.cierresService.cerrarCaja(data);
  }

  @Get('pdf/:id')
  async pdf(@Param('id') id: string, @Res() res: Response) {
    const buffer = await this.cierresService.generarPdfCierre(Number(id));

    res.set({
      'Content-Type': 'application/pdf',
      'Content-Disposition': `inline; filename=cierre-${id}.pdf`,
    });
